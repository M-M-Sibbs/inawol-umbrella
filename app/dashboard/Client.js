...
export default function DashboardClient() {
  ...
  // Derive the login URL from the current dashboard path so a custom
  // ADMIN_PATH works without any public env var.
  const loginPath = () =>
  ...
  async function loadLeads() {
    ...
    if (r.status === 401) { router.replace(loginPath()); return; }
    const d = await r.json();
    setLeads(d.leads || []);
  }
  useEffect(() => { loadProjects(); loadLeads(); }, []); // eslint-disable-line
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (r.status === 401) { router.replace(loginPath()); return; }
      const d = await r.json();
      if (d.url) { set("media_url", d.url); flash("Media uploaded ✓"); }
      else flash(d.error || "Upload failed", "err");
    } catch { flash("Upload failed", "err"); }
  ...
  function startEdit(p) {
    ...
    setForm({
      ...
      problem: p.problem || "", results: p.results || "",
      live_url: p.live_url || "", case_study_url: p.case_study_url || "",
      demo_url: p.demo_url || "", media_type: p.media_type || "image",
      media_url: p.media_url || "", testimonial: p.testimonial || "",
  ...
  async function deleteProject(id) {
    ...
    if (r.status === 401) { router.replace(loginPath()); return; }
    if (r.ok) { flash("Project deleted"); loadProjects(); if (editingId === id) resetForm(); }
  ...
  return (
    <main className="min-h-screen bg-space">
      <div className="border-b border-white/5 bg-space/80 backdrop-blur sticky top-0 z-10">
        <div className="mx-auto max-w-6xl px-5 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display font-bold">Dashboard</span>
          </div>
          <div className="flex gap-2 items-center">
            ...
            <button onClick={logout}
              className="ml-2 rounded-full border border-white/15 px-4 py-1.5 text-sm text-white/70 hover:text-white hover:border-cyanglow/50 transition">
              Sign out
            </button>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8 py-10">
        {msg && (
          <div className={`mb-6 rounded-lg px-4 py-2.5 text-sm border ${
            msgKind === "err"
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                {editingId && (
                  <button onClick={resetForm}
                    className="text-xs text-white/50 hover:text-cyanglow">
                    + New project
                  </button>
                )}
              </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                {projects.map((p) => (
                  <div key={p.id}
                    className={`glass rounded-xl p-4 flex gap-4 transition ${
                      editingId === p.id ? "ring-1 ring-cyanglow/50" : ""
                    }`}>
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <button onClick={() => startEdit(p)}
                        className="rounded-full bg-white/10 hover:bg-cyanglow hover:text-space px-3 py-1 text-xs transition">
                        Edit
                      </button>
                      <button onClick={() => deleteProject(p.id)}
                        className="rounded-full border border-red-500/30 text-red-300 hover:bg-red-500/20 px-3 py-1 text-xs transition">
                        Delete
                      </button>
                    </div>
                  </div>
